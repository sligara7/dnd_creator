...
class CharacterRepository(BaseRepository[Character]):
    ...
    async def get_by_campaign_id(self, campaign_id: UUID) -> List[Character]:
        ...
        query = select(self._model_class).where(
            self._model_class.campaign_id == campaign_id,
            self._model_class.is_deleted == False,  # noqa: E712
        )
        result = await self._session.execute(query)
        return list(result.scalars().all())
    ...
    async def get_by_parent_id(self, parent_id: UUID) -> Optional[Character]:
        ...
        query = select(self._model_class).where(
            self._model_class.parent_id == parent_id,
            self._model_class.is_deleted == False,  # noqa: E712
        )
        result = await self._session.execute(query)
        return result.scalar_one_or_none()

    def to_domain(self, model: Character) -> CharacterDomain:
    ...
    def to_db_model(self, domain: CharacterDomain) -> Character:
