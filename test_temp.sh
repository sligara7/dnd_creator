    ...
    local request_data=$(cat <<EOF
{
    "creation_type": "character",
    "character_id": "$character_id",
    "evolution_prompt": "Refine and improve this character based on user feedback",
    "theme": "$theme",
    "user_preferences": {
...
EOF
    ...
    if [ "${response: -3}" = "200" ]; then
        ...
        if [ "$monster_id" != "null" ] && [ -n "$monster_id" ]; then
            ...
            log_error "Initial monster creation failed - no monster ID returned"
        fi
    else
        log_error "Initial monster creation failed (HTTP ${response: -3})"
        cat /tmp/initial_monster_response.json
    fi
}
    ...
    local request_data=$(cat <<EOF
{
    ...
    "evolution_prompt": "Refine and improve this monster based on user feedback",
    "theme": "$theme",
    "user_preferences": {
        "evolution_type": "refine",
        "preserve_core_concept": true
...
EOF
    ...
    if [ "${response: -3}" = "200" ]; then
        ...
        if [ "$refined_monster_id" != "null" ] && [ -n "$refined_monster_id" ]; then
            ...
            log_error "Monster refinement failed - no monster ID returned"
            echo "$monster_id"  # Return original ID
        fi
    else
        log_error "Monster refinement failed (HTTP ${response: -3})"
        cat /tmp/monster_refinement_response.json
        echo "$monster_id"  # Return original ID
    fi
}
